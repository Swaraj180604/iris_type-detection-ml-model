...
# ── Color helper — Plotly 6+ rejects 8-digit hex; use rgba() instead ─────────
...
with left_col:
    ...
    st.markdown(f"""
    <div class="result-card" style="background:{info['bg']};margin-top:1.5rem;border:1px solid {hex_rgba(color,0.19)};">
    ...
    for sid, sinfo in SPECIES_INFO.items():
        ...
        <div class="prob-row">
          ...
          <div class="prob-track">
            <div class="prob-fill" style="width:{pct}%;background:linear-gradient(90deg,{hex_rgba(sinfo['color'],0.53)},{sinfo['color']});"></div>
          </div>
        </div>
        """, unsafe_allow_html=True)
...
with right_col:
    ...
    with tab1:
        ...
        for sid, sinfo in SPECIES_INFO.items():
            ...
            fig.add_trace(go.Scatterpolar(r=means,theta=fl+[fl[0]],fill='toself',fillcolor=hex_rgba(sinfo["color"],0.1),line=dict(color=sinfo["color"],width=1.5,dash="dot"),name=sinfo["name"],opacity=0.7))
        user_r = [sepal_length,sepal_width,petal_length,petal_width,sepal_length]
        fig.add_trace(go.Scatterpolar(r=user_r,theta=fl+[fl[0]],fill='toself',fillcolor=hex_rgba(color,0.19),line=dict(color=color,width=3),name="Your Input",mode='lines+markers',marker=dict(size=6,color=color)))
    ...
    with tab3:
        ...
        for sid,sinfo in SPECIES_INFO.items():
            sp_df = df[df["species_id"]==sid]
            fig3.add_trace(go.Violin(x=sp_df[dist_feat],name=sinfo["name"],fillcolor=hex_rgba(sinfo["color"],0.25),line_color=sinfo["color"],meanline_visible=True,meanline=dict(color=sinfo["color"],width=2),opacity=0.8,orientation="h",side="positive",width=1.5,points="all",pointpos=-1.5,marker=dict(color=sinfo["color"],size=4,opacity=0.4)))
